...
def adaCost_clf(Y_train, X_train, Y_test, X_test, M, clf, rho=0, C=[1,1.5]):
...
#        print err_m
        ...
        #print err_m
        # Alpha
        alpha_m = 0.5 * np.log((1.0 - err_m)/ err_m)
 ...
 #   print confusion_matrix(Y_train, pred_train)
    ...
    print "Accuracy :"
    thisAcc =  accuracy_score(Y_test, pred_test)
    acc.append(thisAcc)
...
def adaC2_clf(Y_train, X_train, Y_test, X_test, M, clf, rho=0, C=[1,1.5]):
    ...
    tree.export_graphviz(clf_tree,
        out_file='Stumps2/stump_iteration_200_C='+str(C[1])+'.dot') 
    #Create Model
    for i in range(M):
        # Train base classifier with W
        clf.fit(X_train, Y_train, sample_weight = w)
...
def adarecall_clf(Y_train, X_train, Y_test, X_test, M, clf, rho=0, theta=0.1):
    n_train, n_test = len(X_train), len(X_test)
    # Initialize weights
    w = np.ones(n_train) / n_train
    pred_train, pred_test = [np.zeros(n_train), np.zeros(n_test)]
    ...
    for i in range(M):
        ...
        Y_train_copy = [1 if x=='1' else 0 for x in Y_train]
        #print Y_train
        pred_train_i_copy=  [1 if x=='1' else 0 for x in pred_train_i]
        #print pred_train_i
        recallScore = recall_score(Y_train_copy, pred_train_i_copy)
        

        # Error
        #err_m = #float(recallScore)/((1-recallScore) * 100)
        err_m = np.dot(w,miss) / sum(w)
        #err_m = float(sum(miss)) / n_train
        ...
        #print "REcall"
        #print recallScore
        #if(recallScore > err_thr) :
        #    continue

        #print err_m
        # Alpha
        alpha_m = float(recallScore)/(1-recallScore)


        # 0.5 * np.log(((1.0-rho) *(1.0 - err_m) )/ ((1.0+rho)*err_m))
        #print "Alpha"
        #print alpha_m
        #Normalization Factor 
        Z_t = 2*( math.sqrt((err_m*(1-err_m)) / (1-rho**2) ) )
...
if __name__ == '__main__':
    ...
    #test_movie_list =["SchindlersList","GoneGirl","HarryPotter", "Hobbit","LOTR1","ManofSteel","PursuitofHappyness","Avengers-Ultron", "moneyball"]
    test_movie_list = ["Avengers-Ultron"]
    ...
    for test_movie in test_movie_list:
        ...
        #T = [10,50,100,200,300,400,500,550]
        T = {200}
    	...
    	#ros=RandomOverSampler(random_state=0)           #Random over sampling
    	#X_res,Y_res =ros.fit_sample(X_train,Y_train)  
    ...
    print "TN :" + str(tn)
    print "FP :" + str(fp)
    print "Balanced Accuracy : " + str(sum(base_acc)/float(len(base_acc))) 
    print "Accuracy : " + str(sum(acc)/float(len(acc))) 

    print base_acc
    print "Precision : " + str(sum(prec)/float(len(prec))) 
    print "Recall : " + str(sum(rec)/float(len(rec))) 
    print prec
    print rec
    tp = []
    fn = []
